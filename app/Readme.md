...
3. For a new project, the initial prompt is sent to the LLM to generate a fresh multi-file codebase, which is parsed and saved.
4. For an existing project, the flow proceeds in two stages: a planning stage followed by an update stage.
...
6. If the plan response cannot be parsed as JSON, a fallback plan is created using keyword relevance against stored file summaries.
7. In the update stage, the system builds an update prompt (build_update_prompt) that includes the plan and all relevant file contents, and sends it to the LLM.
8. The LLM returns updated and newly generated files in <<<FILE:...>>> blocks, which the system parses and merges into the project.
